...
function handlePlacement(b) {
  ...
  if (g) {
    ...
    const alignedAlongLength = (extendsFront || extendsBack) && insideW && insideH;
    const shouldMerge = alignedAlongLength && isProspectiveLayerFull(g, s);

    if (shouldMerge) {
      // full layer completes -> rebuild combined structure in one pass
